# Thock V2 — Invisible Git: safety & maintainability assessment
...
as a self-contained `thock::history` module driving a new general-purpose
...
- **`crates/thock/src/history.rs`** (new, ~470 lines) — the service:
  ...
  initializing `<vault>/.thock/history` on a background task, with
...
- **`crates/thock/src/vault.rs`** — `[history]` config table
...
- **`assets/settings/default.json`** — `**/.thock/history` added to
  `file_scan_exclusions`.
- **`crates/zed/src/main.rs`** — one line: `thock::history::init(cx)`.
...
Everything else lives in the `thock` crate. No existing function changed
...
probe registered `~/Thock/.thock/history` and the GitStore opened it
...
now skips paths covered by `file_scan_exclusions`, and `**/.thock/history`
...
Thock-specific carve-out. Verified live post-fix: a full
...
Live run (dev build against the author's real `~/Thock` V1 vault):

- Existing V1 vault upgraded transparently: `history/` created lazily, initial
  checkpoint recorded (`checkpoint: initial <utc>`, author `Thock`),
