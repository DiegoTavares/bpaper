# Thock V7 — Dynamic Routines (`routine.toml`) & the generic navigation panel
...
| Provenance | `.thock/areas/<id>/` | `.thock/routines/<id>/` (lockfile + install record) |
...
- **App-owned files migrate silently.** Reconcile moves `.thock/areas/<id>/` → `.thock/routines/<id>/` (provenance is the app's, not the user's).
...
- **Unaffected:** the onboarding marker path (`.thock/state/onboarded/<id>` is id-keyed), skill file locations (`skills/<id>/…`), and the Claude bridges (regenerated from the manifest anyway).
...
9. Any Routine link or skill can be bound to a shortcut via the generic `thock::OpenLink` / `thock::RunSkill` actions (§7.5), addressed by the ids in `routine.toml`.
...
- **No product-name decision.** "Thock" itself and the wider analogy question stay open; this rename settles only the bundle word.

## 5. Core concepts

### 5.1 The vault defines the Routine
V3's installed manifest (`.thock/areas/<id>/manifest.toml`) is an app-owned provenance copy, hidden from the user. V7 makes the definition a **user-space file**: `routines/<id>/routine.toml`. This completes "everything is editable" — not just a Routine's skills and templates, but its very shape (name, links, skill list) is a file the user or their agent can edit, and the panel follows. `.thock/routines/<id>/` shrinks to pure provenance: the hash lockfile and install record.
...
- Materialized pieces (core-owned, scaffolded like `backlog.md`): `skills/thock/new-routine.md` (the ritual) and `routines/ROUTINES.md` (the `routine.toml` format reference, written for agents, with a worked example). Kickoff: `Read and execute skills/thock/new-routine.md`.
- The ritual's contract: interview (domain, files that exist already, desired rituals) → propose the layout → write `routines/<id>/routine.toml`, the doc, `agent_doc`, skill files (under `routines/<id>/skills/`), and any scaffold dirs → **never** touch `.thock/` or `.claude/` → tell the user to activate it in the panel (or write a completion marker à la V5 §5.4 so the app can toast "Finance is ready — activate?").
...
- `thock::OpenLink { routine, link }` — resolves the link (templates included) and dispatches its `kind`-appropriate open; `thock::RunSkill { skill }` — the existing V5 launch path by skill id.
...
4. **Registry strictness is a compat trap.** `deny_unknown_fields` on the config content types (`vault.rs:87, 301, 324`) means any new registry key makes *older builds* treat the whole vault as invalid. This bites twice in V7: the `[[routines.installed]]` key rename (§2 — read both, write new) and the temptation to add per-entry fields. V7 adds **no new registry fields**: origin and hashes live in `.thock/routines/<id>/` (files.lock), which old builds never parse.
...
12. **Keybindable entries (added 2026-08-07, follow-up):** two generic parameterized actions — `thock::OpenLink { routine, link }` and `thock::RunSkill { skill }` — ship in Phase 1 so any Routine entry can get a user shortcut. Manifests never apply keybindings themselves; `ROUTINES.md` documents the snippet.
