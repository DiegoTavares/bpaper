...
All feature logic lives in a new, self-contained crate (`crates/thock/`, four
...
Week, `thock:` open commands, panel open-by-default in vaults, classic
...
| `crates/zed/Cargo.toml` | +1: `thock.workspace = true` | Trivial |
...
| `crates/zed/src/main.rs` | +1 `thock::init(cx)`; −20/+5 in `restore_or_create_workspace` | **The one real conflict surface** (see below) |
| `assets/keymaps/default-{macos,linux}.json` | +1 each: `thock::ToggleFocus` binding | Trivial |
...
there is no session to restore, Thock opens/scaffolds the default vault
...
- **`activation_priority` must be globally unique.** This fork's dock asserts it
  ...
  — the last free small integer. The next Thock panel needs 8+.
...
- `cargo check` + `cargo clippy --deny warnings` clean for `thock` and
...
- Live first-run test (fresh `$HOME`): sample vault scaffolded at
  `~/Thock/` with config, template, and `welcome.md`; workspace opened;
