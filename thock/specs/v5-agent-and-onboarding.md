# Thock V5 — BYO Agent Rails & Agentic Area Onboarding
...
The core bet, locked in the interview: Thock's agent surface is **not a chat UI**. It is a **terminal**. A new right-dock **Agent panel** hosts a real terminal running the user's own CLI agent — Claude Code, Gemini CLI, Codex, anything launchable from a shell — auto-launched in the vault directory with a kickoff prompt passed as a **launch argument**. The full TUI is the product surface: its permission prompts, its `/resume`, its model picker. Thock's job shrinks to three things it can do excellently:
...
| 17 | Pilot Area | **Timeline (retrofit)** — migrate an existing daily-notes system (Obsidian-style vault on disk) into Thock conventions. |

## 3. Goals & success criteria

**Primary (Phase 1):** A user with `claude` (or `gemini`, `codex`, …) installed can connect it once, then click **Run** on any installed skill and watch their own agent execute it in a terminal tab beside their notes — no Zed agent panel, no API keys pasted into Thock, no JSON edited by hand.
...
7. The inherited Zed **agent panel and ACP surfaces are hidden by default** in Thock (config/feature-flag level — no upstream code deleted).
...
- **No session persistence/restore in-app.** Closed tab = gone from Thock; `/resume` in a new conversation is the CLI's affordance, and the onboarding prompt may mention it.
...
- **No MCP connector onboarding** (Milestone 3). The onboarding prompt may *use* MCP servers the user's CLI already has; Thock does not install or configure any.
...
The app cannot see inside the terminal, so completion is communicated through the filesystem — the one channel both sides share. The contract lives in the onboarding file itself: *"When setup is complete, create the file `.thock/state/onboarded/<area_id>` (empty or with a short summary)."* Thock watches that directory. The protocol is best-effort by design; the 24-hour expiry (§8.4) guarantees the UI never nags forever when an agent forgets.
...
**Storage & resolution.** Per-vault override in the vault config (`[agent] command = "..."` in `.thock/config.toml`) wins over the user-level default (Thock settings). If neither exists, the user is not connected.
...
A new GPUI `Panel` in the **right dock** (tabbing alongside the Day Planner Context panel), registered in the `thock` crate with a **unique `activation_priority`**. It reuses the existing terminal infrastructure (`terminal` / `terminal_view` crates) to host **one terminal per session** as panel tabs — the same machinery Zed's agent panel uses for its "Terminal" entries, *reused, not modified*.
...
4. **The completion protocol** — write `.thock/state/onboarded/timeline` when done (a short summary of what was migrated as the file body is encouraged); tell the user Thock will open the capabilities tour.
...
- **Marker appears** (`.thock/state/onboarded/<area_id>`, file-watched): state → onboarded; badge clears; the Area's **explainer doc opens in markdown preview** (once — the moment of the transition, or on next focus if the app was closed).
...
5. **Complete** — write the marker; point at the explainer doc ("Thock will open your Timeline tour").
...
The Timeline **explainer doc** (the v3-required Area doc) is upgraded with a proper walkthrough — what the panel does, Today/Yesterday keystrokes, each skill with a "try it now: Run 'Wrap Today'" pointer, where to edit templates and skills. On the onboarded transition, Thock opens it in the stock **markdown preview** tab. No new rendering surface; "presentation mode" is this, nothing more.

## 8. Implementation notes

- **Crate placement:** panel, connect flow, launch plumbing, and onboarding state machine live in `crates/thock` (with the existing panels). Terminal reuse via the `terminal`/`terminal_view` crates' public APIs; if the agent-panel's terminal-hosting helpers aren't reusable without modification, prefer duplicating the small amount of glue in `thock` over patching upstream crates.
...
- **File watching:** the vault worktree already has watchers; add `.thock/state/onboarded/` to the watched set. Marker handling must be idempotent (marker may pre-exist at startup).
...
- **Session history in-app** — listing past conversations, resuming from Thock rather than `/resume`.
