# Thock V3 — Areas (modular life-domain packages)
...
2. **An "Areas" section** added to the existing Thock dock panel (below the Timeline navigator). Each enabled Area is an entry; clicking it opens the Area's explainer doc **in viewing (rendered) mode**. Under each Area, its **skills** are listed as clickable entries — a first, basic Skills view.
...
**Definition of done:**
1. The sample vault scaffolds with the **Timeline Area pre-installed and enabled**, registered in `.thock/config.toml`, with its files materialized on disk.
2. The Thock left-dock panel shows an **"Areas"** section listing each enabled Area beneath the existing Timeline navigator.
...
- **Catalog** — a set of Areas that **ships inside the Thock binary**. This is the source the "Add Area" list reads from, and the future gallery.
- **Installed** — when an Area is added to a vault, Thock **materializes** it: copies the Area's editable parts (manifest, skills, explainer doc, dashboard/static files) as **real files into the vault**, scaffolds its folders (create-if-missing, never clobbering), and **registers** it in `.thock/config.toml`.

Materialization is what honors two VISION principles at once: **"Everything is editable"** (once installed, an Area's skills and templates are ordinary files the user or their LLM can open and change) and **"Your files, forever"** (the vault is self-describing and complete on disk; if Thock vanished, the Area's files still open in any editor).

### 4.3 The Areas section
Not a new dock panel — a **new section inside the existing Thock panel**, below the Timeline navigator. This keeps the fork surface tiny (no new `activation_priority`, no new `add_panel_when_ready` wiring) and matches the request ("a new header added to the Thock pane for Areas").
...
### 5.4 Config registry (`.thock/config.toml`)
...
- The existing Thock panel (`TimelinePanel`, left dock) renders, **below** the Timeline navigator entries, an **"Areas"** header followed by one row per **enabled** installed Area (registry order).
...
- **Rename consideration:** with two sections in one panel, the panel's persistent name/title stays "Thock"; the two sections are labeled "Timeline" and "Areas" (see the §9 naming flag for the Timeline/Timeline clash).
...
- On selecting one, Thock **materializes** it (all on a background thread, mirroring `ensure_note`):
  ...
  3. Write the installed `manifest.toml` copy under `.thock/areas/<id>/`.
...
### 7.3 The dashboard
- `weekly/site/index.html` — the self-computing static viewer (reads `window.WEEKS`, derives stats, sparklines, lingering-project and carried-over-goal warnings). Shipped as an asset; **not** regenerated by Thock.
...
- **Isolate Area logic in the `thock` crate.** Add an `areas` module (`crates/thock/src/areas.rs`) owning: the manifest schema (serde/`toml`), the catalog registry (app-shipped source), materialize/uninstall, and the modified-since-install diff. The panel calls into it; the delta against Zed's own files stays minimal.
...
- **Extend `scaffold_vault`** (`vault.rs`) so the sample vault installs the Timeline Area at creation time: write the installed manifest, the explainer doc, the skill file, the dashboard assets, and the `[[areas.installed]]` registry entry. The Timeline catalog assets become bundled resources of the `thock` crate.
...
6. **`skills/` location** — top-level `skills/<area>/` (proposed) vs. under `.thock/` vs. the reference vault's `.claude/commands/`. Proposed top-level so skills are visible and editable in the file tree.
7. **Dashboard open target** — always the system browser (§7.4), acceptable given no in-app web view?
8. **Explainer doc filename** — `areas/Timeline.md` (title-cased, human) vs. `.thock/areas/timeline/doc.md` (hidden). Proposed the visible, human-named path.
...
- **Surface:** Areas are a **new section in the existing Thock panel**, not a new dock panel (small fork surface, no new activation priority).
