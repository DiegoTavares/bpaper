...
> This file is written for the AI agent BreadPaper launches from **New
> Routine with AI**. It's a plain skill — open it, edit it, rerun it whenever
> you like.

You are helping the user create a **new Routine** in their BreadPaper vault:
...
4. **Never** write under `.breadpaper/` or `.claude/` — except the single
   ready marker in the final step. BreadPaper generates bridges and
...
1. Write an empty file at `.breadpaper/state/routine-ready/<id>` (create the
   parent directories if needed) — BreadPaper watches for it and offers
