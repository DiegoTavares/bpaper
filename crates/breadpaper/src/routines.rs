...
/// App-owned provenance (`files.lock`) under `.breadpaper/`.
...
pub const NEW_ROUTINE_SKILL_PATH: &str = "skills/breadpaper/new-routine.md";
...
pub struct RoutineLink {
    /// Stable handle for keybindings (`breadpaper::OpenLink`); defaults to
    /// the slugified name.
    pub id: String,
    pub name: String,
...
fn claude_bridge_content(skill: &RoutineSkill) -> String {
    let mut content = format!(
        "---\nname: {name}\ndescription: {description}\ndisable-model-invocation: true\n---\n\n\
         This is a BreadPaper Routine skill. Read and follow the full instructions in\n\
...
/// A Routine package shipped inside the BreadPaper binary.
...
/// `.breadpaper/routines/<id>/files.lock`: the content hash of every declared
...
fn clear_ready_marker(vault_root: &Path, routine_id: &str) {
    ...
    match fs::remove_file(&marker) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {}
        Err(error) => log::warn!(
            "BreadPaper: couldn't remove the ready marker {}: {error}",
            marker.display()
        ),
    }
}
...
/// and records the hash lockfile under `.breadpaper/routines/<id>/`.
    ...
    match existing {
        ...
        Some(existing) => {
            ...
            if recorded_hash == Some(&content_hash(existing.as_bytes())) {
                ...
                log::info!(
                    "BreadPaper: routines/{routine_id}/routine.toml was modified by the user; \
...
/// - `.breadpaper/areas/<id>/` moves to `.breadpaper/routines/<id>/`
...
fn migrate_pre_v7(vault: &Vault) -> Result<()> {
    ...
    if legacy_dir.is_dir() {
        ...
        if let Err(error) = fs::remove_dir(&legacy_dir) {
            log::debug!(
                "BreadPaper: left {} behind after migration: {error}",
                legacy_dir.display()
            );
        }
    }
...
fn migrate_installed_routine(
        ...
        let mut relocate = |source: &str, new_destination: &mut String| -> Result<()> {
            ...
            if unmodified && !new_path.exists() {
                ...
                *new_destination = old_rel.clone();
                kept_in_place = true;
                log::info!(
                    "BreadPaper: {old_rel} was modified (or its new home exists); the migrated \
    ...
    match fs::remove_file(&legacy_manifest_path) {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {}
        Err(error) => log::warn!(
            "BreadPaper: couldn't remove {}: {error}",
        ...
        let load = match load_vault_manifest(&vault.root, &entry.id) {
            Ok(Some(manifest)) => {
                for warning in &manifest.warnings {
                    log::warn!("BreadPaper: routines/{}/routine.toml: {warning}", entry.id);
                }
...
pub fn delete_routine(vault_root: &Path, routine_id: &str) -> Result<RemovalOutcome> {
    ...
    if installed_dir.exists() {
        ...
        // pruning starts at its parent (`.breadpaper/routines/`). The walk
        // stops at `.breadpaper/` because config.toml keeps it non-empty.
...
mod tests {
    ...
    fn scaffold_preinstalls_timeline_routine() {
        ...
        // Provenance: the hash lockfile under .breadpaper/routines/.
...
"#,
    ...
    /// The §2 migration: a pre-V7 vault (old registry key, `.breadpaper/
...
"#;
    ...
    fn onboarding_marker_lives_under_state_dir() {
        ...
        assert_eq!(
            ...
            root.join(".breadpaper/state/routine-ready/finance")
        );
    }
