...
// panel's rename.
const ROUTINES_PANEL_KEY: &str = "BreadPaperTimelinePanel";

actions!(
    breadpaper,
    [
        /// Toggles focus on the BreadPaper routines panel.
        ToggleFocus,
...
/// `["breadpaper::OpenLink", { "routine": "timeline", "link": "today" }]`.
...
impl RoutinesPanel {
    ...
    fn refresh_routines(&mut self) {
        ...
        self.addable_catalog = match routines::catalog() {
            Ok(catalog) => catalog
                ...
                .collect(),
            Err(error) => {
                log::error!("BreadPaper: couldn't load the Routines catalog: {error:?}");
                Vec::new()
            }
        };
    }
    ...
    fn open_note(&mut self, entry: TimelineEntry, window: &mut Window, cx: &mut Context<Self>) {
        let VaultStatus::Valid(vault) = &self.vault_status else {
            ...
            self.show_toast_deferred(
                ...
                "This workspace isn't a BreadPaper vault.".to_string(),
                cx,
            );
            return;
        };
    ...
    /// `breadpaper::OpenLink { routine, link }`: dispatch by the stable ids
    ...
    fn render_invalid_routine(
        ...
        ListItem::new(ElementId::Name(SharedString::from(format!(
            "breadpaper-invalid-routine-{routine_id}"
        ))))
        .start_slot(
    ...
    fn render_add_routine(&self, cx: &Context<Self>) -> impl IntoElement {
        let mut list = v_flex().child(
            ListItem::new("breadpaper-add-routine")
            ...
            for discovered in &self.discovered {
                list = list.child(match &discovered.manifest {
                    ...
                    Err(error) => {
                        ...
                        ListItem::new(ElementId::Name(SharedString::from(format!(
                            "breadpaper-discovered-invalid-{routine_id}"
                        ))))
                        .indent_level(1)
                        .indent_step_size(px(12.))
                        .start_slot(
        ...
        list = list.child(
            ListItem::new("breadpaper-new-routine")
                .indent_level(1)
                .indent_step_size(px(12.))
                .start_slot(
...
impl Render for RoutinesPanel {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        key_context.add("BreadPaperRoutinesPanel");
        key_context.add("menu");
        v_flex()
            .id("breadpaper-routines-panel")
